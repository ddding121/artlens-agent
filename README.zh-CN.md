...
**版本：0.1.2，本地研究原型。** 已实现图片上传、OpenAI 兼容视觉接口、可选 CLIP 图片检索、馆藏来源、保守未知判断和会话追问。未接入音乐；未训练自有模型；尚无真实识别准确率。它是有界工具工作流，尚不是模型自主规划任意工具调用的完整 agent。
...
- 空阈值默认使用相似度 0.95、领先幅度 0.03，现有 .env 可覆盖。仅用于请求分流，未经校准。
- 达到门槛后将上传图和本地第一名馆藏参考图交给视觉 API 比较，增加一次请求和费用。
- 只有有效的 same 判断、至少两条具体对应细节且没有矛盾时标为“很可能匹配”，展示馆藏作者与年代。
- 图片缺失、接口失败或证据不足保持待核验；明显不同时返回未知。不进行真伪鉴定。
- v0.1.2 将标题和列表正常排版，显示核验说明、对应细节、差异、候选领先幅度和真实耗时。
- 等待期间只显示实际经过的时间，不伪造百分比或内部步骤。
- 模型复核仍可能错误；来源级逐条事实检查尚未实现，不能把引用提示词当作真实性保证。

### 从已上传的旧版本更新

停止服务，确认本地没有未提交修改后，从对应更新分支拉取（分支名见本次 GitHub PR）。保留 .env、.venv 和 data，无需重建索引或安装新依赖。启动后按 Ctrl+F5；/api/health 的 version 应为 0.1.2。
